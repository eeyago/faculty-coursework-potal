...
{
	$pwde = md5($_POST['password']);
mysql_query("UPDATE lectures SET lecname='$_POST[lecname]',     gender='$_POST[gender]', address='$_POST[address]', courseid='$_POST[course]' 	contactno='$_POST[contactno]' WHERE lecid = '$_POST[lecid]'");
...
<form name="form1" method="post" action="" id="formID">
  ...
  <label for="lecname">Lecturer Name</label>
    <input type="text" name="lecname" id="lecname" class="validate[required,custom[onlyLetterSp]] text-input" value="<?php if(isset($lecname)) { echo $lecname; } else { echo ""; } ?>">
  </p>
  <p>
  ...
  ?> class="validate[required] radio" />
    ...
    <input type="radio" name="gender" id="radio2" value="Female"  <?php if(isset($gender))
  {
	if($gender == "Female")
  	{
	  	echo "checked";
  ...
  <p>
    ...
    <textarea name="address" id="address" class="validate[required]" cols="25" rows="5"><?php if(isset($address)) { echo $address; } else { echo ""; } ?></textarea>
  </p>
  <p>
    <label for="contactno">Contact No</label>
    <input type="text" name="contactno" id="contactno" class="validate[required,custom[phone]] text-input"  value="<?php if(isset($contactno)) { echo $contactno; } else { echo ""; } ?>">
  </p>
  <p>
